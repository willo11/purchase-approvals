 ...
 * {@link EvidenceGeneratorPort} + {@link EvidenceStorePort}. PR #4 injected a
 * stub; PR #5 replaces it with the real PdfGenerator and the S3 evidence
 * store (composition root — tests inject fakes instead).
 */
const requests = makeRequestRepository();
const approvers = makeApproverRepository();
const gate = new ApproverGate(requests, approvers);
const evidence = new PdfGenerator();
const evidenceStore = makeEvidenceStore();

export const approve = buildApprove(
  new ApproveRequest(gate, approvers, requests, evidence, evidenceStore)
);
