...
- **Fresh-review hardening (PR #4 fixes)**:
  - **Cross-direction single winner (CRITICAL)**: the completion CAS was `attribute_not_exists(completedAt)` only — an empirical `Promise.all([completeIfAbsent, rejectIfPending])` probe won BOTH ~40% of the time (reject lands, then complete flips `REJECTED → COMPLETED`, leaving `completedAt` AND `rejectedAt`). The condition is now **`attribute_not_exists(completedAt) AND #status = :pending`** — symmetric with the reject CAS — so a REJECTED request can never be completed and the two flags never coexist. A repo-level integration test races both writers over 10 iterations asserting exactly-one-winner + completed XOR rejected.
  - **Completion trigger liveness**: `DynamoDbRequestRepository.get()` reads the approver set with a `QueryCommand`; it now uses **`ConsistentRead: true`** so a signer ALWAYS observes a concurrent peer's committed Step A write. Without it, two concurrent signers could each read ≤2 signed and neither would reach the completion CAS — the request would stick `PENDING` forever. (The Step A per-approver CAS already prevented double-signs; consistency was a liveness, not safety, issue.)
  - **Validated-OTP precondition (spec R1/R2 → 401)**: `ValidateOtp` on SUCCESS writes a durable `validatedAt` on the APPR row (`ApproverRepository.markValidated`); `ApproveRequest`/`RejectRequest` require it BEFORE Step A (`OtpNotValidatedError` → 401), keeping the OTP endpoints unregressed (issue/validate/regenerate do NOT require it). Tradeoff noted: the marker does NOT expire — a stale validation still unlocks acting (acceptable for the demo; release disclaimer planned PR #8).

---

## 23. PDF evidence: pdf-lib + Helvetica, deterministic-key idempotency, inline generation, S3 store
- **Tradeoff (a) — PDF library**: pdf-lib vs pdfmake vs pdfkit inside a Lambda runtime.
  - **Decision**: `PdfGenerator` (task 5.1) uses **pdf-lib with `StandardFonts.Helvetica`** — pure TypeScript, ZERO native dependencies (Decision 7 implemented). The signature table is drawn explicitly as 3 rows; the "Requester" line reads `createdBy.name` from the SNAPSHOT (never re-fetched from the registry), so evidence is immutable even if identity data changes later (Decision 3).
  - **Why**: native binaries are a real risk inside Lambda — pdf-lib avoids them entirely; standard fonts need no embedding bloat.
  - **Interview line**: "The PDF is generated with pdf-lib and a standard Helvetica font — no native deps, so it runs in Lambda without bundling binaries."
- **Tradeoff (b) — evidence idempotency**: a redelivered/double execution of the completion path must not double-generate or double-record.
  - **Decision**: TWO complementary guards, both keyed on the deterministic `evidenceKey = reqs/<id>/evidence.pdf` (`evidenceKeyFor`, spec R2):
    1. **Read guard**: `maybeComplete` returns early when the REQ row already carries `evidenceKey` — a replay never re-generates.
    2. **Write guard**: `recordEvidence` is a conditional `UpdateItem` with `ConditionExpression: attribute_not_exists(evidenceKey)` — even if two writers raced, the key is set at most once.
  - Combined with the completion CAS (`attribute_not_exists(completedAt)`), exactly one writer ever runs generate → S3 put → record. This is the interview's "existence key" story (design-concurrency §5): the deterministic key means a retry OVERWRITES the same object instead of duplicating it.
- **Tradeoff (c) — inline vs event-driven generation**: generate the PDF synchronously inside the 3rd-signature Lambda vs fan out to SQS/SFN/EventBridge and generate asynchronously.
  - **Decision**: **INLINE for the demo** — the completion CAS winner generates and uploads synchronously, inside the Lambda (documented in design-concurrency §5 and flagged in apply-progress). No SQS/SFN now.
  - **Why**: one fewer moving part for the take-home; the generation is a single pdf-lib call + one S3 put, well inside the Lambda timeout; the CAS already serializes the trigger.
  - **Documented evolution**: the `EvidenceGeneratorPort` + `EvidenceStorePort` seams mean an SQS queue / Step Functions state machine can be introduced LATER with zero changes to the domain: the completion handler enqueues `{requestId}`, a worker consumes and runs the same generate → put → record path. The guard `attribute_not_exists(evidenceKey)` makes the worker idempotent across redeliveries (the exact class of problem SQS retries create).
  - **Interview line**: "Generation is inline today because it's cheap and the CAS already single-winners the trigger; the port seam lets me move it behind SQS later without touching the domain."
- **Tradeoff (d) — evidence store**: S3 vs DynamoDB objects vs external storage.
  - **Decision**: **S3** (`S3EvidenceStore`, task 5.2) under the deterministic key with `ContentType: application/pdf`; IAM already scoped to `s3:GetObject`/`s3:PutObject` on the evidence bucket.
  - **Why**: PDFs are large binary blobs — DynamoDB has a 400 KB item limit and charges per RCU/WCU; S3 is the canonical object store and API Gateway can serve binary bodies from it (spec R3).
  - **Testability seam (local/CI)**: `EvidenceStorePort` (`put`/`get`) is the contract; production wires `S3EvidenceStore`, integration tests inject an **in-memory store that returns REAL PDF bytes** — the whole generate → put → get → download flow is tested end-to-end against dynamodb-local WITHOUT LocalStack. Same seam trick as the mail outbox (Decision 21): the domain never knows a bucket exists.
- **Gotcha**: `GET /api/purchase-requests/{id}/evidence.pdf` returns 404 for every "no PDF" case (request missing / not COMPLETED / no `evidenceKey` / object gone) — including after a generation failure, where the request KEEPS `COMPLETED` (spec R4). There is deliberately no "retry generation" endpoint in this PR; a failed generation stays 404 until a successful one exists.
...
- **Decision**:
  ...
  2. **`POST /api/purchase-requests/{id}/approvers/{email}/recover`** → the requester-initiated recovery. **Gate chain**: request exists (404), global state not terminal (`COMPLETED`/`REJECTED` → 410), approver present in the request (404), approver IS locked (else 409).
