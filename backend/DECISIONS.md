...
- **Tradeoff (c) — 3-strike lockout atomicity**: naive read-then-increment can overshoot or race.
  ...
  - **Costs / nuance**: the increment and the lockout-flag write are two UpdateItem calls; the atomic guarantee that matters (never overshoot, only the 3rd-failure writer locks, idempotent lock) is held by the singular `attempts < 3` CAS, and the lockout write is conditional on still being `ACTIVE` so a stale caller cannot relock gracefully. The OTP digest is SHA-256 of `code + context` — the plaintext code is never stored or compared.
