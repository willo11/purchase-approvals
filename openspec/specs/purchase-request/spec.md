...
Define the purchase request aggregate: creation from registered users, listing, detail, per-approver records, and the global state model that dominates the whole flow.

Dependencies: `user-registry`. Requester and approver emails MUST reference registered users; at creation the system snapshots each user's name for `createdBy` and each approver. Approval tokens and simulated mails at creation time are provided by `approver-otp`; approve/reject transitions are defined in `approval-signature`.
