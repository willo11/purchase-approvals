# Approval Signature Specification

## Purpose

Define approve/reject use-cases, signature records keyed by approver email, terminal-state rules, and atomic state transitions under concurrency.

Dependencies: `purchase-request` data model and global state model (approver records keyed by email as `SK=APPR#<email>`); a validated OTP (from `approver-otp`) is required to act.

## Requirements

### R1. Approve

Given a validated OTP, the system MUST record a signature for that approver: the APPROVER'S REGISTERED name (from the `user-registry` snapshot persisted at request creation) + a signing timestamp. The name MUST NOT be typed by the approver at signing time. The system MUST set the approver status to `SIGNED` and persist the signed timestamp.

#### Scenario: Approver signs with registered name

- GIVEN an approver with a validated OTP on a `PENDING` request whose snapshot name is "Ana"
- WHEN the approver approves
- THEN the approver's status is `SIGNED` with the registered name "Ana" and a timestamp
- AND no name input is requested from the approver

### R2. Reject is globally terminal

Given a validated OTP, the system MUST set the approver status to `REJECTED` with timestamp AND set the global request status to `REJECTED` immediately. After any rejection, every other approver link MUST be blocked (informational "already rejected"), and no approve/reject can succeed.

#### Scenario: Rejection terminates the request

- GIVEN a request with 3 pending approvers
- WHEN one approver rejects
- THEN the request status is `REJECTED` and the rejecting approver's status is `REJECTED`

#### Scenario: Other approvers blocked after rejection

- GIVEN a request already `REJECTED`
...
When the 3rd approver signs, the system MUST set the global status to `COMPLETED` and trigger PDF evidence generation (`pdf-evidence`).

#### Scenario: Third signature completes the request

- GIVEN a request with 2 signed approvers
- WHEN the 3rd approver signs
- THEN the global status becomes `COMPLETED` and PDF generation is triggered
...
- THEN exactly one is the 3rd signature that completes the request, both signatures are recorded, and the global status is `COMPLETED`
