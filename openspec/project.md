...
A requester creates a purchase request (title, description, amount, 3 approvers). The system stores it with status "Pendiente", generates a unique UUID token per approver, and simulates an email containing an approval link. An approver validates a time-limited OTP (valid for 3 minutes), reviews the purchase details, and approves or rejects. Each approval records a digital signature (name + date) and the request status moves to "Firmado" / "Rechazado". When all 3 approvers have signed, the backend generates a PDF evidence document (request data + approvers/signatures table), stores it, and exposes a download endpoint; the request status becomes "Completada".
